...
async function getPokemonAnterior(numero) {
    ...
    if (pokemonAnterior != null) return `<button class="btn btn-outline-danger btn-10" onclick='drawPokemon(${pokemonAnterior.id})'>
    ${pokemonAnterior.id.toString().padStart(3, "0")}<br>${capitalizeFirstLetter(pokemonAnterior.name)}</button>`
